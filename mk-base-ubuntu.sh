...
if [[ "$TARGET" == "gnome-full" ||  "$TARGET" == "xfce-full" ]]; then
    ...
    echo "LANGUAGE=en_US:en:en_US:en" >> /etc/environment

    echo "export LC_ALL=en_US.UTF-8" >> /etc/profile.d/en_US.sh
    echo "export LANG=en_US.UTF-8" >> /etc/profile.d/en_US.sh
    echo "export LANGUAGE=en_US:en:en_US:en" >> /etc/profile.d/en_US.sh
...
# Create User
useradd -G sudo -m -s /bin/bash armsom
passwd armsom <<IEOF
armsom
armsom
IEOF
gpasswd -a armsom video
gpasswd -a armsom audio
passwd root <<IEOF
root
root
IEOF
